...
export default function Home() {
  ...
  const handleExifUpload = async (e: any) => {
    ...
    const processFiles = files.map((file: any) => {
      return new Promise<void>((resolve) => {
        ...
        EXIF.getData(file, function (this: any) {
          ...
          let latDecimal = 35.6812 + (Math.random() - 0.5) * 0.05;
          let lonDecimal = 139.7671 + (Math.random() - 0.5) * 0.05;

          if (lat && lon) {
            let ddLat = lat[0] + lat[1] / 60 + lat[2] / 3600;
            if (EXIF.getTag(this, 'GPSLatitudeRef') === "S") ddLat *= -1;
            latDecimal = ddLat;

            let ddLon = lon[0] + lon[1] / 60 + lon[2] / 3600;
            if (EXIF.getTag(this, 'GPSLongitudeRef') === "W") ddLon *= -1;
            lonDecimal = ddLon;
          } else {
            noGpsCount++;
          }
          ...
          parsedSpots.push({
            ...
            lat: latDecimal,
            lng: lonDecimal,
            timestamp: dateTime ? new Date().toISOString() : new Date(file.lastModified).toISOString(),
            mediaUrl: URL.createObjectURL(file),
            isVideo: false
          });
          resolve();
