...
> = {
  ...
  AL: { name: 'アルバニア (Albania)', flag: '🇦🇱', region: '🇪🇺 ヨーロッパ', lat: 41.1533, lon: 20.1683, zoom: 8 },
  BA: { name: 'ボスニア (Bosnia)', flag: '🇧🇦', region: '🇪🇺 ヨーロッパ', lat: 43.9159, lon: 17.6791, zoom: 8 },
  ME: { name: 'モンテネグロ (Montenegro)', flag: '🇲🇪', region: '🇪🇺 ヨーロッパ', lat: 42.7087, lon: 19.3744, zoom: 9 },
...
- 右下の「🎯（現在地）」ボタンを押すと、あなたの今いる場所に移動します。
- 右下の「🪟（引き戻す）」ボタンを押すと、都道府県から国・世界全体へと段階的に視野を広げることができます。

2. 写真・動画の投稿方法
- 画面下の「📷＋ 写真 / 動画を追加」ボタンを押して、アルバムからメディアを選びます。
...
export default function WorldSnapApp() {
  ...
  useEffect(() => {
    const hasCompleted = localStorage.getItem('ws_onboarded_v11');
    if (hasCompleted) {
