...
第3条（位置情報の取得・利用とプライバシー）
・当サービスでは、ユーザーが現在地を表示する機能（「現在地へ移動」ボタン）および写真に付与された位置情報（EXIFデータ）を読み取るために、お使いの端末の位置情報を取得および利用します。
・取得した位置情報は、マップへのピン留めや足跡マップ機能の提供にのみ使用され、本人の同意なく第三者に公開されることはありません。

第4条（不適切なコンテンツへの対処・モデレーション）
...
1. マップ機能の使い方
- 地図上で世界中の旅のスポットを閲覧できます。
- マップ上を「ダブルタップ」すると、その場所を中心に素早くズームイン（拡大）できます。
- 右下の「🎯（現在地）」ボタンを押すと、お使いの端末の位置情報アクセス許可の確認画面が表示され、許可するとあなたの今いる現在地に素早く移動します。
- 右下の「🪟（引き戻す）」ボタンを押すと、都道府県から国・世界全体へと段階的に視野を広げることができます。

2. 写真・動画の投稿方法
- 画面下のクローム（📷＋ 写真 / 動画を追加）ボタンを押して、アルバムからメディアを選びます。
- 撮影場所の「地名・住所検索」で場所を指定し、公開範囲（ワールド・フレンド・マイマップ）やカテゴリ（View・グルメ・雨の日）を選んで「マップに反映する」を押すとピンが立ちます。
- 動画を投稿した際も、詳細画面のプレーヤーで音声付きで再生できます。

3. 足跡マップ機能
- マイページの「足跡マップ」タブを開くと、あなたが訪れた国のリストが表示されます。
- 国名をタップすると、その国にあるあなたの投稿スポット周辺（市区町村サイズ：半径約10km）がオレンジ色にハイライトされます。

4. 投稿の自動翻訳機能
- 投稿の詳細画面にある「🌐 翻訳する」ボタンを押すと、あなたが設定した表示言語に一瞬で文章が翻訳されます。もう一度押すと元の文章に戻ります。

5. 設定・メニューの活用
- 左上の「☰（メニュー）」からいつでも表示言語や初期マップのベース国を変更できます。また利用規約やブロックリストの管理もここから行えます。`;
...
export default function WorldSnapApp() {
  ...
  useEffect(() => {
    const hasCompleted = localStorage.getItem('ws_onboarded_v12');
    if (hasCompleted) {
  ...
  const handleCompleteOnboarding = () => {
    localStorage.setItem('ws_onboarded_v12', 'true');
    setIsOnboarding(false);
