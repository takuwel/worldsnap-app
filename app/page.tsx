...
const GUIDE_FULL_TEXT = `【WorldSnap の操作説明と使い方ガイド（関西弁モードやで！）】

1. 現在地にひとっとび！「🎯ボタン」
- マップ画面の右下にある「🎯（現在地ボタン）」をポチッと押すと、スマホの画面に「位置情報のアクセス許可」の確認が出るで！
- そこで「許可」を選ぶと、一瞬で「あんたの今おるところ」へ地図がビュンと飛んで現在地を中心にしてくれるからめっちゃ便利やで！

2. マップをぐりぐり動かそう！
- マップを「ダブルタップ」すると、その場所が大きく拡大（ズームイン）するで。
- 右下の「🪟（引き戻すボタン）」を押したら、都道府県から国、そして世界全体の大きな地図へスムーズに戻れるで！

3. 写真や動画をポンとのせよう！
- 下の「📷＋ 写真 / 動画を追加」ボタンを押して思い出のメディアを選ぶねん。
- 「地名・住所検索」で場所を指定して、公開範囲（ワールド・フレンド・マイマップ）やカテゴリを選んで投稿したら、地図にピンがパーンと立つで！動画も音声付きでバッチリ再生できるで！

4. 足跡マップと自動翻訳
- マイページの「足跡マップ」で国をタップすると、その周辺がオレンジ色にぬりつぶされるで！
- 外国語の投稿も「🌐 翻訳する」ボタンを押したら一瞬で言葉が翻訳されるから安心や！`;
...
export default function WorldSnapApp() {
  ...
  useEffect(() => {
    const hasCompleted = localStorage.getItem('ws_onboarded_v14');
    if (hasCompleted) {
  ...
  const handleCompleteOnboarding = () => {
    localStorage.setItem('ws_onboarded_v13', 'true');
    setIsOnboarding(false);
